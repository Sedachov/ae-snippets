(function createNullsFromPaths (thisObj) {
    ...
    function buildUI(thisObj) {
        ...
        var firstButton = localize("$$$/AE/Script/CreatePathNulls/PathPointsToNulls=Points Follow Nulls (grab tangents)");
        ...
        forEachPath(function(comp,selectedLayer,path){
            ...
            for (var i = 0, ii = pathPoints.length; i < ii; i++){ //For each path point
                ...
                if(comp.layer(nullName) == undefined){
                    ...
                    //Add points to control tangents
                    var t_in_slider = newNull.Effects.addProperty("Point Control");
                        t_in_slider.name = 'tangent_IN';
                        t_in_slider.property("Point").setValue(path.value.inTangents[i]);
                    var t_out_slider = newNull.Effects.addProperty("Point Control");
                        t_out_slider.name = 'tangent_OUT';
                        t_out_slider.property("Point").setValue(path.value.outTangents[i]); 
            ...
            path.expression =
                        ...
                        "    L = getNullLayers[i]; \r" +
                        "    if (L != null && L.index != thisLayer.index){ \r" +  
                        "        try{ t_in = L.effect(\"tangent_IN\")(1); } catch(err){ t_in = [0,0]; } \r" +
                        "        try{ t_out = L.effect(\"tangent_OUT\")(1); } catch(err){ t_out = [0,0]; } \r" +
                        "        origPoints[i] = fromCompToSurface(L.toComp(L.anchorPoint));  \r" +
                        "        origInTang[i] = L.toComp(t_in) - L.toComp(L.anchorPoint);  \r" +
                        "        origOutTang[i] = L.toComp(t_out) - L.toComp(L.anchorPoint);  \r" +
                        "    }} \r" +
